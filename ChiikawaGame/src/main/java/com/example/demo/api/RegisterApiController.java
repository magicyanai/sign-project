...
public class RegisterApiController {
	...
	public ResponseEntity<?> register(@RequestBody RegisterBean registerBean){
		...
		// **Step 1: 驗證 reCAPTCHA**
        String recaptchaResponse = registerBean.getRecaptchaResponse();
        boolean isCaptchaValid = recaptchaService.validateCaptcha(recaptchaResponse);
		
        // 如果 reCAPTCHA 驗證失敗，返回錯誤訊息
        if (!isCaptchaValid) {
            return ResponseEntity.badRequest().body(
                Map.of("status", "fail", "errors", List.of("無效的 reCAPTCHA 驗證"))
            );
        }
        
        // **Step 2: 呼叫 Service 層進行註冊處理**
